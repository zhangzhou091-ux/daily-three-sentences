          ...
          <div className="apple-card p-6 bg-blue-600 text-white">
            ...
            <div className="space-y-3 text-sm">
              <div>
                ...
                <p className={`font-mono break-all ${getSupabaseConfig().URL ? 'text-white' : 'text-blue-200/50'}`}>
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <span className={`font-bold ${getSupabaseConfig().ANON_KEY ? 'text-white' : 'text-blue-200/50'}`}>
                    {getSupabaseConfig().ANON_KEY ? '✅ 已配置' : '❌ 未配置'}
                  </span>
                  {getSupabaseConfig().ANON_KEY && (
                    <span className="text-xs bg-white/20 px-2 py-0.5 rounded">
                      {getSupabaseConfig().ANON_KEY.length} 字符
                    </span>
                  )}
                </div>
              </div>
              <div>
                ...
                <div className="flex gap-2">
                  <span className={`px-3 py-1 rounded-full text-xs font-bold ${getSupabaseConfig().URL && getSupabaseConfig().ANON_KEY ? 'bg-green-500 text-white' : 'bg-red-500 text-white'}`}>
                    {getSupabaseConfig().URL && getSupabaseConfig().ANON_KEY ? '✅ 完整' : '❌ 不完整'}
                  </span>
                </div>
              </div>
            </div>
