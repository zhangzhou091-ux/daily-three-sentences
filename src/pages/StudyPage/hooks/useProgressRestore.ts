...
export const useProgressRestore = ({
  ...
  useLayoutEffect(() => {
    ...
    const currentSentenceExists = currentSentence && currentSentence.id === savedProgress.sentenceId;

    if (currentSentenceExists) {
      console.log(`✅ 学习进度前置恢复成功: 索引 ${currentIndex}`);
      return;
    }
    ...
    if (foundIndex >= 0 && foundIndex < dailySelection.length) {
      ...
      console.log(`✅ 学习进度校正成功: 索引 ${foundIndex}`);
    } else {
      console.log(`📍 进度索引超出范围，重置到第一个`);
      setCurrentIndex(0);
    }
  }, [activeTab, dailySelection, currentIndex, setCurrentIndex, isMountedRef, setProgressAdjusted]);
