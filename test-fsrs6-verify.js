...
console.log('FSRS-6 验证测试 - 模拟原始测试逻辑\n');
...
const sameSequence = results.filter(r => r.ratings.join('') === '333333');

console.log(`\n评分序列为 G→G→G→G→G→G (333333) 的句子共 ${sameSequence.length} 个:\n`);

for (const r of sameSequence) {
  console.log(`序号 ${r.sentenceIndex + 1}:`);
  console.log(`  评分序列: ${r.ratings.map(getRatingShort).join(' → ')}`);
  console.log(`  间隔序列: ${r.intervals.join(' → ')}`);
  console.log(`  最终稳定性: ${r.finalStability.toFixed(6)}`);
  console.log(`  最终难度: ${r.finalDifficulty.toFixed(6)}`);
  console.log();
}

if (sameSequence.length > 1) {
  const first = sameSequence[0];
  let allSame = true;
  for (let i = 1; i < sameSequence.length; i++) {
    if (sameSequence[i].finalStability !== first.finalStability ||
        sameSequence[i].finalDifficulty !== first.finalDifficulty) {
      allSame = false;
      break;
    }
  }
  console.log(`\n所有 ${sameSequence.length} 个 G→G→G→G→G→G 句子的结果是否完全一致: ${allSame ? '✅ 是' : '❌ 否'}`);
}
